...
// ── Leitura/escrita genérica de um JSON do repo (ex: companies.json) ──────────
...
export async function readJson(path) {
  ...
  const json = await res.json()
  let parsed = null
  try { parsed = JSON.parse(Buffer.from(json.content, 'base64').toString('utf8')) } catch { /* ignore */ }
...
// Dispara um workflow (workflow_dispatch). Requer PAT com Actions: write.
export async function dispatchWorkflow(workflowFile, inputs) {
  ...
  if (!res.ok) {
    const err = new Error('workflow_dispatch_failed')
    err.status = res.status
    err.detail = await res.text().catch(() => '')
    throw err
  }
